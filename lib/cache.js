...
/*
 * This method is a helper to get all subdomains for wildcard usage
 */
Cache.prototype.getDomains = function(hostname, recursing) {
    var idx = hostname.indexOf('.');
    var lastIdx = hostname.lastIndexOf('.');

    if (idx == lastIdx) {
        return recursing ? null : [ hostname ];
    } else {
        return [ hostname.substr(idx), this.getDomains(hostname.substr(idx + 1), true) ];
    }
}
...
Cache.prototype.getBackendFromHostHeader = function (host, callback) {
    ...
    var readFromCache = function (hostKey, cb) {
        ...
        var domains = this.getDomains(hostKey, false);
        for (var i = 0; i < domains.length; i += 1) {
            if (domains[i]) {
                // If domain null (the terminator), this is skipped
                multi.lrange('frontend:*' + domains[i], 0, -1);
            }
        }
    ...
    readFromCache(host, function (rows) {
        // The last element will always be the deads list
        var deads = rows.pop();
        var backends;
