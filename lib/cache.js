...
Cache.prototype.getBackendFromHostHeader = function (host, callback) {
    ...
    readFromCache(host, function (rows) {
        ...
        var frontendFound = backends.shift();
        while (rows.length && !backends.length) {
            backends = rows.shift();
            frontendFound = backends.shift();
        }
        if (!backends.length) {
        ...
        if (backend.host.indexOf(this.config.server.wildstring) > -1) {
          backend.host = backend.host.replace(this.config.server.wildstring, host.replace(frontendFound.substr(frontendFound.indexOf('*') + 1, frontendFound.length), ''));
          backend.hostname = backend.hostname.replace(this.config.server.wildstring, host.replace(frontendFound.substr(frontendFound.indexOf('*') + 1, frontendFound.length), ''));
          backend.href = backend.href.replace(this.config.server.wildstring, host.replace(frontendFound.substr(frontendFound.indexOf('*') + 1, frontendFound.length), ''));
        }
